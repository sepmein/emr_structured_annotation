...
class PneumoniaNERModel(LabelStudioMLBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.finetuned_model_path = os.getenv("FINETUNED_MODEL_PATH", "gliner2-multi-v1")
        self.threshold = float(os.getenv('THRESHOLD', 0.4))
    ...
    def _lazy_init(self):
        ...
        try:
            ...
            model_path = str(pathlib.Path(self.MODEL_DIR, self.finetuned_model_path))
            logger.info(f"--- [DEBUG] 正在从 {model_path} 加载 GLiNER2 模型 ---")
            self.__class__._gliner = GLiNER2.from_pretrained(model_path)
        except Exception as e:
            logger.error(f"Failed to load local model: {e}")
            logger.info(f"Loading GLiNER2 model from HuggingFace: {GLINER_MODEL_NAME}")
            self.__class__._gliner = GLiNER2.from_pretrained(GLINER_MODEL_NAME)
    
        # 预构建 schema
        self.__class__._schema = self.__class__._gliner.create_schema().entities({
