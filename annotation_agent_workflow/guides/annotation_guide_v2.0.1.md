...
版本：v2.0.1  
发布日期：2026-08-28  
状态：v2.0.0病例规则不变；已扩展逐标签医学定义；生产使用前须完成第15章真人一致性试验  
...
| 真人试验Schema锁 | `schema_manifest_v2.0.1.json`；同一批不得混用当前XML与未批准推荐标签 |
| 操作字典 | `label_dictionary_v2.0.1.md`；每个标签均含医学定义、项目边界、正反例和关系要求，必须与本指南共同使用 |
...
5. 已知支气管扩张等结构性肺病，出现近期咳嗽/咳痰/气促明显加重，并有发热、低氧、湿啰音等客观证据之一；
...
10. 支气管扩张伴感染稳定复诊，无近期症状加重或客观急性证据。

#### D. 信息不足

文本为空、严重截断、字段错位，或仅有无法归属的项目/数值，使目标、待复核和非目标均无法可靠判断。只有“病情稳定、继续治疗”等泛化语句，却没有疾病名称、症状或检查，也判信息不足。
...
4. 是否有疑似肺炎、急性发热加呼吸证据、结构性肺病急性恶化、性质不明肺/胸异常，或呼吸道标本中明确检出关注病原体？有 → 待专业复核。
5. 其余 → 非目标。
...
- 近期咳嗽/咳痰/气促明显加重，并伴发热、低氧、湿啰音等至少一项客观证据：待专业复核。
...
本章保留供流程学习使用的标签概览。实际画实体或连关系前，标注员必须查阅 `label_dictionary_v2.0.1.md` 中对应标签卡。每张卡把“医学上是什么”与“本项目在原文中标什么”分开；医学定义不能用于补造原文未表达的事实。
...
| 只有“病情稳定/继续治疗”等泛化内容 | 信息不足；不猜测所治疗疾病 |
...
4. 疑似肺炎，或急性发热+呼吸证据，或结构性肺病急性恶化，或当前肺/胸异常性质不明，或呼吸道标本中明确检出关注病原体？→ 待专业复核。
