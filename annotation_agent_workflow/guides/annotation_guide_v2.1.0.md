...
3. 索引事件中有性质或活动性不明的肺/胸部异常，包括影像异常，或当前咳嗽/咳痰等呼吸症状合并双肺湿啰音等客观肺部听诊异常；此入口不要求同时存在发热。孤立湿啰音、单纯呼吸音粗或已有明确非感染性替代解释时不自动触发；
4. 呼吸道标本中检出本项目关注的呼吸道病原体，但没有足够肺炎直接证据；
5. 已知支气管扩张等结构性肺病，原文把咳嗽/咳痰/气促明显加重锚定到本次就诊、当前连续疗程或明确复诊原因，并有发热、低氧、湿啰音等客观证据之一；“今年加重多次”等年度计数、仅写未改善/仍有、换抗生素或单独一个低氧数值，均不能替代“本次明确加重＋客观证据”；
6. 慢性或非感染性解释不确定，同时存在新发/加重呼吸症状或新的肺部异常；
...
1. 文本是否可用？不可用 → 信息不足。
2. 是否有明确慢性/非感染性语境或明确替代病因？有 → 先按第3.2C判断非目标；若同时存在独立急性证据，继续第3步。
...
- 血清IgM/抗体阳性不能等同呼吸道标本检出；单独出现不触发待专业复核，须结合索引事件中的急性呼吸证据。
...
本模型表达的是“原文时间跨度 + 相对索引事件的位置 + 病程状态”，足以支持当前病例级目标。它不负责：
...
100例压力测试没有发现必须再增加人工时间标签的案例；主要问题来自索引事件归属和源文档边界，应优先修数据和指南，而不是增加按钮。
...
- 现病史/入院情况：默认属于患者和索引事件，除非句内明确写既往、计划、否定或他人。
- 既往史：默认是非索引过去事实；实体仍按XML标，能定位提示词时连接“既往”。
- 家族史/接触者描述：默认不属于患者；只有原文明确涉及项目关注证据时标实体并连接“与患者本人无关”。
- 治疗计划/建议：未实施内容连接“假设性”；不能当成已发生检测、治疗效果或诊断。
- 出院记录：同次住院早期明确事实仍属于索引事件；“好转/已缓解”改变活动性，不删除该次事实。
- 复制模板或“相关后文”：只有来源和索引事件归属明确时使用；与当前段矛盾且影响结论时退回数据修复。
...
- 多个就诊/文书被拼接但没有来源、时间和索引事件标识，且自动规则无法安全拆分。
...
5. 按患者、就诊流水、activity/source和时间拆分索引事件；
6. 检验按原始记录ID成组输出；
7. 生成排除日志和少量门禁回归样本。
...
- 病例级：若该句是脱离就诊语境的纯检验列表，判信息不足；若它附在一份完整、明确无关的临床记录中，判非目标；若同一索引事件另有急性呼吸证据，判待专业复核。任何情况下都不能仅凭该结果判目标病例。
